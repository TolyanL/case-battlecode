...
class Course(models.Model):
    ...
    # --- ИЗМЕНЕНО: related_name для избежания конфликта ---
...
class Quest(models.Model):
    ...
    # --- НОВОЕ ПОЛЕ: Связь с Course ---
    course = models.ForeignKey(
        Course,
        on_delete=models.SET_NULL, # Или CASCADE, если квест не может существовать без курса
        null=True,
        blank=True,
        related_name="quests_in_course", # Имя для обратной связи
        verbose_name="Курс",
    )
    # --- /НОВОЕ ПОЛЕ ---
    difficulty = models.CharField(
